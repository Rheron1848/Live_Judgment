...
| 6 | 接受高刷屏房间 DOM 裁剪丢消息的现状（裁剪只丢最旧消息，检测窗口内通常仍在）；不够用时再实现 WS 通道 | 用户 | 2026-08-16 |
| 7 | 接受 DOM 属性耦合 B 站页面结构的改版失效风险；失效后修选择器（已集中在 dom-chat-source.ts 一处） | 用户 | 2026-08-16 |
...
- 无。原待决 1（DOM 裁剪丢消息）与待决 2（DOM 属性耦合）已于 2026-08-16 由用户拍板接受现状，见「已拍板决策」6、7。
